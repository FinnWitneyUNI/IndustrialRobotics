classdef move_robot < handle
    methods (Static)
        function robot()
            ...
            % Define inverse kinematics parameters
            weights = [1, 1, 1, 1, 1, 1];  % Adjust weights as needed
            initialGuess = r.model.getpos();  % Use the home configuration as an initial guess
            
            % Launch the external GUI for brick selection and teaching the robot
            disp('Launching brick selection GUI...');
            select_brick_gui(r.model, weights, initialGuess);  % Pass the model and IK parameters to the GUI
